...
function parseCommitEntry(entry: string): RawSidecar['commits'][number] {
  ...
  return {
    ...
    changes: changeLines.map(parseChangeLine).filter(isPresent),
    id,
...
function parseChangeLine(line: string) {
  ...
  return { kind: 'modify' as const, path };
}
